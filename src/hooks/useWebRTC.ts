...
export const useWebRTC = (socket: Socket | null) => {
  ...
  const captureMedia = async (
    ...
    try {
      ...
      if (mediaType === 'video') {
        ...
        const encodings = [
          { rid: 'r0', scalabilityMode: 'S3T3', maxBitrate: 1500000 }, // Set a reasonable max bitrate
        ];
        ...
        try {
          ...
          if (track.readyState === 'live') {
            ...
            await transport.produce({
              track,
              encodings: [
                { dtx: false, scalabilityMode: 'S3T3', maxBitrate: 1500000 } // Set a reasonable max bitrate
              ] // Simplified encoding
            });
          } else {
  ...
  const createRecvTransport = useCallback(async (data: NewConsumerEventData) => {
    ...
    const transportEventName = `transportCreated_recv_${data.kind}_${data.producerId}`;
      ...
      try {
        ...
        socket.once(consumeReadyEventName, async (rtpCapabilities: mediasoupClient.types.RtpParameters) => {
          try {
            ...
            console.log(`${data.kind} track settings for peer ${data.peerId}:`, consumer.track.getSettings());
    ...
    socket.emit('createTransport', {
      ...
      kind: data.kind,
      peerId: data.peerId,
      recvProducerId: data.producerId // Include producer ID for better tracking
    });
